...
async def main(args: argparse.Namespace, out_dir: Path) -> None:
    ...
    with tqdm(total=4, desc="09 open_positions", unit="step", dynamic_ncols=True) as pbar:
        # ── Total count ────────────────────────────────────────────────
        ...
        for row in by_side.iter_rows():
            side, n, upnl, pct = row
            print(f"  {str(side):<8}  n={n:>8,}  total uPnL=${upnl:>+14,.2f}  profitable={pct:.1%}")
        print()
        pbar.update()
        ...
        print(f"  Profitable positions:  {profitable:>8,}  ({profitable / len(upnl) * 100:.1f}%)")
        print(f"  Underwater positions:  {int((upnl < 0).sum()):>8,}  ({int((upnl < 0).sum()) / len(upnl) * 100:.1f}%)")
        ...
        if "firstOpenedAt" in df.columns:
            age_hours = ((pl.lit(_NOW_TS) - df["firstOpenedAt"]) / 3600.0).drop_nulls()
            _print_box("POSITION AGE (hours since firstOpenedAt)")
            print(f"  Mean age:    {age_hours.mean():>8.1f} h  ({age_hours.mean() / 24:.1f} days)")
            print(f"  Median age:  {age_hours.median():>8.1f} h  ({age_hours.median() / 24:.1f} days)")
            print(f"  P90 age:     {age_hours.quantile(0.9):>8.1f} h  ({age_hours.quantile(0.9) / 24:.1f} days)")
            print(f"  Max age:     {age_hours.max():>8.1f} h  ({age_hours.max() / 24:.1f} days)")
            age_buckets = [
                ("<1h",   age_hours < 1),
        ...
        print(f"  Wallets with 1 position:   {int((wallet_counts['n_positions'] == 1).sum()):>8,}")
        print(f"  Wallets with 2-5 positions:{int(((wallet_counts['n_positions'] >= 2) & (wallet_counts['n_positions'] <= 5)).sum()):>8,}")
        print(f"  Wallets with >5 positions: {int((wallet_counts['n_positions'] > 5).sum()):>8,}")
        print()

        _print_box("TOP 10 LARGEST UNDERWATER POSITIONS")
        worst = df.filter(pl.col("unrealizedPnl").is_not_null()).sort("unrealizedPnl").head(10)
        print(f"  {'Wallet':<44} {'Asset':<10} {'Side':<6} {'uPnL':>14}")
        print("  " + "-" * 78)
        for row in worst.select(["ownerAccount", "asset", "side", "unrealizedPnl"]).iter_rows():
            wallet, asset, side, upnl = row
            print(f"  {str(wallet)[:42]:<44} {str(asset):<10} {str(side):<6} ${upnl:>+13,.2f}")
        print()
        pbar.update()
